 ...
 * 2025-03
 * Scout4BRecoSecondaryVertexNMCAnalyzer can produce secondary vertex any number of muon Track or Tracks
...
class Scout4BRecoSecondaryVertexNMCAnalyzer : public edm::one::EDAnalyzer<>
{
public:
   explicit Scout4BRecoSecondaryVertexNMCAnalyzer(const edm::ParameterSet &iConfig);
   ~Scout4BRecoSecondaryVertexNMCAnalyzer() override;
...
//
Scout4BRecoSecondaryVertexNMCAnalyzer::Scout4BRecoSecondaryVertexNMCAnalyzer(const edm::ParameterSet &iConfig)
...
Scout4BRecoSecondaryVertexNMCAnalyzer::~Scout4BRecoSecondaryVertexNMCAnalyzer() = default;

//
// Begin and end job
//
void Scout4BRecoSecondaryVertexNMCAnalyzer::beginJob()
{
...
void Scout4BRecoSecondaryVertexNMCAnalyzer::endJob()
{
   clearVars();
}
...
//
void Scout4BRecoSecondaryVertexNMCAnalyzer::analyze(edm::Event const &iEvent, edm::EventSetup const &iSetup)
{
   // Init the branches
   clearVars();
...
// Funtions definition
//
UInt_t Scout4BRecoSecondaryVertexNMCAnalyzer::getTriggerBits(const edm::Event &iEvent)
{
   UInt_t trigger = 0;
...
float Scout4BRecoSecondaryVertexNMCAnalyzer::distanceOfClosestApproach(const reco::Track *track1, const reco::Track *track2, const MagneticField &bFieldHandle)
{
...
Measurement1D Scout4BRecoSecondaryVertexNMCAnalyzer::distanceOfClosestApproach(const reco::Track *track, const reco::Vertex &vertex, const MagneticField &bFieldHandle)
{
...
KinematicFitResult Scout4BRecoSecondaryVertexNMCAnalyzer::KinematicFitter(std::vector<reco::Track *> trks, std::vector<double> masses, const MagneticField &bFieldHandle)
{
...
KinematicFitResult Scout4BRecoSecondaryVertexNMCAnalyzer::MCKinematicFitter(std::vector<reco::Track *> trks, std::vector<double> masses, double MMass, double MMassErr, const MagneticField &bFieldHandle)
{
   // https://twiki.cern.ch/twiki/bin/view/CMSPublic/SWGuideKinematicVertexFit
   if (trks.size() != masses.size())
      throw cms::Exception("Error") << "number of tracks and number of masses should match";
...
Scout4B::Displacements Scout4BRecoSecondaryVertexNMCAnalyzer::compute3dDisplacement(const KinematicFitResult &fit, bool closestIn3D)
{
...
std::pair<std::vector<KinematicFitResult>, std::vector<KinematicFitResult>> Scout4BRecoSecondaryVertexNMCAnalyzer::performVertexFit(
    std::vector<reco::Track> tracks,
    unsigned int NP,
    int MCharge,
    double MMass,
...
std::pair<std::vector<XFitResult>, std::vector<KinematicFitResult>> Scout4BRecoSecondaryVertexNMCAnalyzer::performXVertexFit(
    std::vector<reco::Track> tracks,
    unsigned int NM,
    const std::vector<std::vector<KinematicFitResult>> &vertexs,
    int XCharge,
...
void Scout4BRecoSecondaryVertexNMCAnalyzer::clearVars()
{
   // Clear All Variables
   nXcand = 0;
   nMmu = 0;
...
// Fill Descriptions
//
void Scout4BRecoSecondaryVertexNMCAnalyzer::fillDescriptions(edm::ConfigurationDescriptions &descriptions)
{
   ...
   descriptions.add("Scout4BRecoSecondaryVertexNMCAnalyzer", desc);
}

// declare this class as a framework plugin
DEFINE_FWK_MODULE(Scout4BRecoSecondaryVertexNMCAnalyzer);