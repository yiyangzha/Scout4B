...
using namespace Scout4B;

Matrix33 Scout4B::jacobianSphToCart(const reco::Candidate::LorentzVector& p4)
{
  Matrix33 jacobian;
...
FreeTrajectoryState Scout4B::getFTS(const reco::Photon& photon,
	    double energyErr,
...
KinematicParticleRef Scout4B::build_particle(const reco::Photon& photon,
	       double energyErr,
	       const MagneticField * field,
	       float& chi2,
	       float& ndof)
...
LorentzVector Scout4B::makeLorentzVectorFromPxPyPzM(double px, double py, double pz, double m){
  double p2 = px*px+py*py+pz*pz;
  return LorentzVector(px,py,pz,sqrt(p2+m*m));
}

LorentzVector Scout4B::makeLorentzVectorFromP3M(const math::XYZVector& p, double m){
  return LorentzVector(p.x(), p.y(), p.z(), sqrt(p.Mag2() + m * m));
}

LorentzVector Scout4B::makeLorentzVectorFromP3M(const GlobalVector& p, double m){
  return LorentzVector(p.x(), p.y(), p.z(), sqrt(p.mag2() + m * m));
}
